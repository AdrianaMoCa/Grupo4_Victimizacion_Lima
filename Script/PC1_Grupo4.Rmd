...
lima= lima |> mutate(victima=
                  case_when(C300_P4 %in% c("1")~"Sí",
                            C300_P4 %in% c("2")~"No",
                            TRUE~'NA'))
lima$victima <- factor(lima$victima,
                       levels = c("Sí", "No"))
```

```{r}
lima |> 
  ggplot(aes(x= victima,  group=Zona)) + 
...
lima= lima |> mutate(sexo=
                  case_when(C200_P7 %in% c("1")~"Hombre",
                            C200_P7 %in% c("2")~"Mujer",
                            TRUE~'NA'))
lima$victima <- factor(lima$victima,
...
```{r}
vic_sexo <- lima[lima$C300_P4 %in% c("1"),]

vic_sexo |> 
  ggplot(aes(x= sexo,  group= C300_P4)) + 
  geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") +
  geom_text(aes( label = scales::percent(..prop..),y= ..prop.. ), stat= "count", vjust = -.5) +
...
lima= lima |> 
  mutate(robo_vehiculo=
           case_when(C300_P5_1 %in% c("0")~"No",
                            C300_P5_1 %in% c("1")~"Sí",
                            FALSE~'NA'))

lima$robo_vehiculo <- factor(lima$robo_vehiculo,
                       levels = c("Sí", "No", FALSE~'NA'))
...
```{r}
vic_vehi <- lima[lima$C300_P5_1 %in% c("1"),]

vic_vehi |> 
  filter(!is.na(C300_P5_1)) |> 
  ggplot(mapping = aes(x=sexo ,  group=C300_P5_1)) + 
  geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") +
  geom_text(aes( label = scales::percent(..prop..),
                   y= ..prop.. ), stat= "count", vjust = -.5) + labs(y = "Percent", fill="¿Usted cree que puede ser víctima del robo de su vehículo?") + facet_grid(~Zona) + scale_y_continuous(labels=percent)+labs(title="Porcentaje de victimización por robo de vehículo", x="¿Usted cree que puede ser víctima del robo de su vehículo?")+ scale_fill_brewer(palette = "Set1")+theme(legend.position = "bottom")+theme(legend.position = "none")
```
...
```{r echo=FALSE}
lima= lima |> 
  mutate(lima,robo_vivienda=
                  case_when(C300_P5_2 %in% c("0")~"No",
                            C300_P5_2 %in% c("1")~"Sí",
                            FALSE~'NA'))
...
```{r}
vic_viv <- lima[lima$C300_P5_2 %in% c("1"),]

vic_viv |> 
  filter(!is.na(C300_P5_2)) |> 
  ggplot(lima, mapping = aes(x= sexo,  group=C300_P5_2)) + 
  geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") +
  geom_text(aes( label = scales::percent(..prop..),
                   y= ..prop.. ), stat= "count", vjust = -.5) + labs(y = "Percent", fill="¿Usted cree que puede ser víctima del robo de su vehículo?") + facet_grid(~Zona) + scale_y_continuous(labels=percent)+labs(title="Porcentaje de victimización por robo de vivienda", x="¿Usted cree que puede ser víctima del robo de su vivienda?")+ scale_fill_brewer(palette = "Set1")+theme(legend.position = "bottom")+theme(legend.position = "none")
```
...
lima= lima |> 
  mutate(confianzapnp=
                  case_when(C300_P11A_1 %in% c("1")~"Nada Confiable",
                            C300_P11A_1 %in% c("2")~"Poco Confiable",
                            C300_P11A_1 %in% c("3")~"Algo Confiable",
                            C300_P11A_1 %in% c("4")~"Muy Confiable",
                            FALSE~'NA'))

lima$confianzapnp <- factor(lima$confianzapnp, levels = c("Muy Confiable", "Algo Confiable", "Poco Confiable", "Nada Confiable", FALSE~'NA'))
...
```{r}
lima |> 
  filter(!is.na(confianzapnp)) %>% 
  ggplot(lima, mapping = aes(x=Zona,fill=confianzapnp))+geom_bar(position="fill")+labs(y="Porcentaje encuestados/as",x="Grupos de edad")+coord_flip()+ scale_y_continuous(labels=percent)+labs(title="Confianza en la PNP", x="Zona de Lima Metropolitana")+ scale_fill_brewer(palette = "Set1")+theme(legend.position = "bottom")+ theme(legend.title = element_blank())
```
